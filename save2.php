...
//$HISTORY_IMAGE_PATH = rawurldecode( $_POST['HISTORY_IMAGE_PATH'] );
$path = 'http://amsapp.net/images/';


$IMG_DATA = base64_decode($_POST['HISTORY_IMAGE_PATH']);
...
//$IMG_DATA = $_POST['HISTORY_IMAGE_PATH'];
//$HISTORY_IMAGE_PATH = base64_decode($IMG_DECODE);
//$path = 'http://amsapp.net/images/'.$HISTORY_ASSET_ID.$HISTORY_DAY.$HISTORY_MONTH.$HISTORY_YEAR.$HISTORY_HOUR.$HISTORY_MINUTE.'.jpg';
//file_put_contents($path,$HISTORY_IMAGE_PATH);
//$file = fopen($path, 'wb');
//$is_written = fwrite($file, $HISTORY_IMAGE_PATH);
//fclose($file);
...
HISTORY_MINUTE,HISTORY_STATUS_NAME,HISTORY_USERNAME,HISTORY_NOTE,HISTORY_IMAGE_PATH)
...
$query = mysqli_query($conn, "SELECT * FROM HISTORY_ASSET_RECENT1 WHERE HISTORY_ASSETID='$HISTORY_ASSET_ID'");
...
*/
		if( $INPUT_TIME >= $STORE_TIME ){
			$query = mysqli_query($conn, "UPDATE HISTORY_ASSET_RECENT1 SET
    ...
    $query = mysqli_query($conn, "INSERT INTO HISTORY_ASSET_RECENT1 (HISTORY_ASSETID,HISTORY_ASSET_NAME,HISTORY_REFERID,
    	HISTORY_REFERNAME,HISTORY_RECEIVEDATE,HISTORY_SPEC,
    	HISTORY_UNITNAME,HISTORY_BUILDING_ID,HISTORY_BUILDING_NAME,
    	HISTORY_FLOOR_ID,HISTORY_ROOM_ID,HISTORY_DAY,
    	HISTORY_MONTH,HISTORY_YEAR,HISTORY_HOUR,
